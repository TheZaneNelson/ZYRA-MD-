   ...
   if (pairingCode && !GlobalTechInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +2348094718285")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +2348094718285 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
    ...
    GlobalTechInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./Zyra-bug")(GlobalTechInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
GlobalTechInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
	        ...
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: The.Zane.Nelson`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: The.Zane.Nelson`))
            console.log(chalk.magenta(`${themeemoji} WA NUMBER: ${owner}`))
            console.log(chalk.magenta(`${themeemoji} CREDIT: The.Zane.Nelson`))
        }
        if (
            connection === "close" &&
            lastDisconnect &&

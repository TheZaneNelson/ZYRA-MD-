...
const readline = require("readline");
    ...
    const ZyraColor = color[Math.floor(Math.random() * color.length)];
...
async function XeonProject() {
    ...
    try {
        ...
        const phoneNumber = await question(ZyraColor + 'Enter target number🤙 : ' + xColor);
        
        // Request the desired number of pairing codes
        const xeonCodes = parseInt(await question(ZyraColor + 'Amount 😽 : '+ xColor));
        ...
        for (let i = 0; i < xeonCodes; i++) {
            try {
                ...
                console.log(ZyraColor + `${phoneNumber} [${i + 1}/${xeonCodes}]`+ xColor);
            } catch (error) {

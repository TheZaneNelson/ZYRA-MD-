...
exports.buffergif = async (image) => {
	...
	const filename = `${Math.random().toString(36)}`
			await fs.writeFileSync(`./ZYRA/trash/${filename}.gif`, image)
					 child_process.exec(
								`ffmpeg -i ./ZYRA/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./ZYRA/trash/${filename}.mp4`
										) 
  await sleepy(4000)
  
	var buffer5  =  await  fs.readFileSync(`./ZYRA/trash/${filename}.mp4`)
	Promise.all([unlink(`./ZYRA/video/${filename}.mp4`), unlink(`./ZYRA/gif/${filename}.gif`)])
	return buffer5
				   }